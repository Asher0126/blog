...
<script>
import { getTypeDetail, createType, updateType } from '@/api/type'
export default {
  props: {
    dialogStatus: {
      type: String,
  ...
  watch: {
    visible (val) {
      if (val) {
        // 修改的时候，获取详情数据
        if (this.dialogStatus === 'update') { this.getTypeDetail() }
  ...
  methods: {
     ...
     */
    async getTypeDetail () {
      // console.log('getTypeDetail', this.id)
      const result = await getTypeDetail(this.id)
